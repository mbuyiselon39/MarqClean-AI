      ...
      <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div>
            ...
            <button className="rounded-full bg-slate-950 px-4 py-2 text-xs font-semibold text-white transition hover:bg-indigo-700" onClick={() => importRef.current?.click()}>Import file</button>
          </div>
        </div>
        {sources.length > 0 ? (
          <div className="mt-3 flex flex-wrap gap-2 text-xs">
            {sources.map((s) => (
              <button
                ...
                className={`rounded-full border px-3 py-1.5 font-medium ${s.name === primaryName ? "border-indigo-500 bg-indigo-50 text-indigo-700" : "border-slate-300 text-slate-600 hover:border-indigo-400"}`}
              >
        ...
        <>
                ...
                {steps.map((s, i) => (
                  <li key={s.id} className="flex items-center justify-between gap-3 rounded-lg bg-slate-50 px-3 py-2">
                    ...
                    <span className="flex shrink-0 items-center gap-1">
                      <button className="rounded border border-slate-300 px-1.5 text-xs text-slate-500 hover:border-indigo-500" onClick={() => moveStep(s.id, -1)} aria-label={`Move step ${i + 1} up`}>↑</button>
                      <button className="rounded border border-slate-300 px-1.5 text-xs text-slate-500 hover:border-indigo-500" onClick={() => moveStep(s.id, 1)} aria-label={`Move step ${i + 1} down`}>↓</button>
          ...
          <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-5">
            ...
            <div className="mt-3 flex flex-wrap items-end gap-3 text-xs">
              ...
              <button className="rounded-full bg-indigo-600 px-5 py-2 text-xs font-semibold text-white transition hover:bg-indigo-700" onClick={addStep}>Add step</button>
            </div>
          </div>
      ...
      <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div>
            ...
            <button className="rounded-full bg-slate-950 px-4 py-2 text-xs font-semibold text-white transition hover:bg-indigo-700" onClick={() => importRef.current?.click()}>Import table</button>
          </div>
        </div>
        ...
        <>
          <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-5">
            ...
            <div className="mt-3 flex flex-wrap items-end gap-3 text-xs">
              ...
              <button className="rounded-full bg-indigo-600 px-5 py-2 text-xs font-semibold text-white transition hover:bg-indigo-700" onClick={addRelationship}>Add relationship</button>
            </div>
          ...
          {relationships.length > 0 ? (
            <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-5">
              ...
              <div className="mt-3 flex flex-wrap items-end gap-3 text-xs">
                ...
                <button className="rounded-full bg-indigo-600 px-5 py-2 text-xs font-semibold text-white transition hover:bg-indigo-700" onClick={addMeasure}>Add measure</button>
              </div>
              {measures.length > 0 ? (
          ...
          {measures.length > 0 ? (
            <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-5">
              ...
              <div className="mt-3 flex flex-wrap items-end gap-3 text-xs">
                ...
                <button className="rounded-full bg-slate-950 px-5 py-2 text-xs font-semibold text-white transition hover:bg-indigo-700" onClick={buildModel}>Build model</button>
              </div>
              {result ? (
                <div className="mt-4">
  ...
  return (
    <svg viewBox={`0 0 ${width} 160`} className="mt-3 w-full" role="img" aria-label="Relationship diagram between tables">
      {relationships.map((rel) => {
        ...
        return (
          <g key={rel.id}>
            <line x1={from.x} y1={from.y} x2={to.x} y2={to.y} stroke={hasError ? "#e11d48" : "#6366f1"} strokeWidth={2} />
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        ...
        <div className="flex flex-wrap gap-2 text-xs">
          {(["All", ...DAX_CATEGORIES] as const).map((c) => (
            <button key={c} onClick={() => setCategory(c)} className={`rounded-full border px-3 py-1.5 font-semibold ${category === c ? "border-indigo-500 bg-indigo-50 text-indigo-700" : "border-slate-300 text-slate-600"}`}>{c}</button>
          ))}
        </div>
      </div>
