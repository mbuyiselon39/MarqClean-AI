  ...
  return (
    <div className="ws-root ws-scope relative min-h-screen">
      ...
      <main id="main-content" className="relative mx-auto max-w-6xl px-5 pb-20 pt-24 lg:px-8">
        ...
        {error ? <p className="mt-5 rounded-xl bg-surface p-4 text-sm font-medium text-error" role="alert">{error}</p> : null}

        {/* Step 1: Upload */}
        <section className="mt-6 rounded-xl border border-line bg-canvas p-5">
          <h2 className="text-lg font-medium">1. Upload Excel File</h2>
          <div
            className="mt-3 rounded-xl border border-dashed border-line bg-surface p-6 text-center transition hover:border-line"
        ...
        {workbook && activeSheet ? (
          <section className="mt-6 rounded-xl border border-line bg-canvas p-5">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              <label className="text-xs font-medium uppercase tracking-wide text-ink-3">Worksheet
                <select className="mt-1 block w-full rounded border border-line px-2 py-1.5 text-sm font-normal" value={sheetName} onChange={(e) => { setSheetName(e.target.value); const d = autoDetect(workbook.sheets[e.target.value]); setHeaderRow(d.headerRow); setStartRow(d.startRow); setColumns(d.columns); }}>
                  {workbook.sheetNames.map((n) => <option key={n} value={n}>{n}</option>)}
                </select>
              </label>
              <label className="text-xs font-medium uppercase tracking-wide text-ink-3">Header row
                <input type="number" min={1} className="mt-1 block w-full rounded border border-line px-2 py-1.5 text-sm font-normal" value={headerRow} onChange={(e) => setHeaderRow(Math.max(1, Number(e.target.value)))} />
              </label>
              <label className="text-xs font-medium uppercase tracking-wide text-ink-3">Data start row
                <input type="number" min={1} className="mt-1 block w-full rounded border border-line px-2 py-1.5 text-sm font-normal" value={startRow} onChange={(e) => setStartRow(Math.max(1, Number(e.target.value)))} />
              </label>
              <label className="text-xs font-medium uppercase tracking-wide text-ink-3">Date / Note column
                <select className="mt-1 block w-full rounded border border-line px-2 py-1.5 text-sm font-normal" value={columns.dateCol} onChange={(e) => setColumns({ ...columns, dateCol: Number(e.target.value) })}>{colOptions.map((i) => <option key={i} value={i}>{colLabel(i)}</option>)}</select>
              </label>
              <label className="text-xs font-medium uppercase tracking-wide text-ink-3">Client Name / Reference column
                <select className="mt-1 block w-full rounded border border-line px-2 py-1.5 text-sm font-normal" value={columns.clientCol} onChange={(e) => setColumns({ ...columns, clientCol: Number(e.target.value) })}>{colOptions.map((i) => <option key={i} value={i}>{colLabel(i)}</option>)}</select>
              </label>
              <label className="text-xs font-medium uppercase tracking-wide text-ink-3">SRN / Account Reference column
                <select className="mt-1 block w-full rounded border border-line px-2 py-1.5 text-sm font-normal" value={columns.srnCol} onChange={(e) => setColumns({ ...columns, srnCol: Number(e.target.value) })}>{colOptions.map((i) => <option key={i} value={i}>{colLabel(i)}</option>)}</select>
              </label>
              <label className="text-xs font-medium uppercase tracking-wide text-ink-3">Amount column
                <select className="mt-1 block w-full rounded border border-line px-2 py-1.5 text-sm font-normal" value={columns.amountCol} onChange={(e) => setColumns({ ...columns, amountCol: Number(e.target.value) })}>{colOptions.map((i) => <option key={i} value={i}>{colLabel(i)}</option>)}</select>
              </label>
            </div>
            ...
            {preview ? (
              <div className="mt-5">
                ...
                <p className="mt-3 text-xs text-ink-3">Worksheet: {preview.sheetName} | Header row: {preview.headerRow} | Start row: {preview.startRow} | Detected columns: {preview.headers.join(", ")}</p>
                <div className="mt-3 overflow-auto rounded-xl border border-line" style={{ maxHeight: "22rem" }}>
                  <table className="min-w-full text-left text-sm">
                    <thead className="bg-surface text-xs uppercase tracking-wide text-ink-3"><tr><th className="px-3 py-2">Date / Note</th><th className="px-3 py-2">Client Name</th><th className="px-3 py-2">SRN</th><th className="px-3 py-2">Amount</th><th className="px-3 py-2">Grouping key</th></tr></thead>
                    <tbody className="divide-y divide-slate-100">
                      {preview.sampleRows.map((row, i) => (
                        <tr key={i}><td className="px-3 py-1.5 text-ink-2">{row.date || "-"}</td><td className="px-3 py-1.5 text-ink-2">{row.client || "-"}</td><td className="px-3 py-1.5 text-ink-2">{row.srn || "-"}</td><td className="px-3 py-1.5 text-ink-2">{row.amount}</td><td className="px-3 py-1.5 font-medium text-accent">{row.key}</td></tr>
                      ))}
                    </tbody>
                  </table>
                </div>
        ...
        {preview ? (
          <section className="mt-6 rounded-xl border border-line bg-canvas p-5">
            <h2 className="text-lg font-medium">3. Execute Sorting</h2>
            <p className="mt-1 text-sm text-ink-2">Runs grouping, sorting, subtotaling, workbook generation, and client-side validation. The download only unlocks after the generated file is re-opened successfully.</p>
            <button className="mt-4 rounded-md bg-accent-tint px-6 py-3 text-sm font-medium text-ink transition hover:bg-accent-tint" onClick={execute} disabled={busy}>
        ...
        {validation && !validation.ok ? (
          <section className="mt-6 rounded-xl border border-line bg-surface p-5" role="alert">
            <p className="text-lg font-medium text-error">Workbook validation failed. The generated Excel file is invalid.</p>
            <ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-error">
              {validation.issues.map((iss, i) => <li key={i}>{iss}</li>)}
            </ul>
            <p className="mt-3 text-xs text-error">Sheets attempted: {validation.sheetSummaries.map((s) => `${s.name} (${s.rows} rows)`).join(", ") || "none"} | Binary size: {validation.size} bytes. Download is blocked until validation passes.</p>
          </section>
        ) : null}
        ...
        {processed && downloadInfo && validation?.ok ? (
          <section className="mt-6 rounded-xl border border-line bg-canvas p-5">
            <h2 className="text-lg font-medium">4. Download Sorted Workbook</h2>
            ...
            {showErrors && processed.errors.length ? (
              <div className="mt-4 overflow-auto rounded-xl border border-line" style={{ maxHeight: "22rem" }}>
                <table className="min-w-full text-left text-sm">
                  <thead className="bg-surface text-xs uppercase tracking-wide text-ink-2"><tr><th className="px-3 py-2">Row</th><th className="px-3 py-2">A</th><th className="px-3 py-2">B</th><th className="px-3 py-2">C</th><th className="px-3 py-2">D</th><th className="px-3 py-2">Reason</th></tr></thead>
                  <tbody className="divide-y divide-amber-100">
                    {processed.errors.map((e, i) => (
                      <tr key={i}><td className="px-3 py-1.5">{e.sourceRow}</td><td className="px-3 py-1.5">{e.a}</td><td className="px-3 py-1.5">{e.b}</td><td className="px-3 py-1.5">{e.c}</td><td className="px-3 py-1.5">{e.d}</td><td className="px-3 py-1.5 text-ink-2">{e.reason}</td></tr>
                    ))}
                  </tbody>
                </table>
              </div>
                ...
                <table className="min-w-full text-left text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {processed.groups.slice(0, 30).flatMap((g, gi) => [
                      ...g.rows.map((row, ri) => (
                        <tr key={`${gi}-${ri}`}><td className="px-3 py-1.5 text-ink-2">{row.date || ""}</td><td className="px-3 py-1.5 text-ink-2">{row.client}</td><td className="px-3 py-1.5 text-ink-2">{row.srn}</td><td className="px-3 py-1.5 text-right text-ink-2">{formatAmount(row.amount)}</td></tr>
                      )),
                      <tr key={`${gi}-sub`} className="bg-surface"><td className="px-3 py-1.5" /><td className="px-3 py-1.5" /><td className="px-3 py-1.5" /><td className="px-3 py-1.5 text-right font-medium text-ink">{formatAmount(g.subtotal)}</td></tr>,
                      <tr key={`${gi}-blank`}><td className="px-3 py-1.5" colSpan={4}>&nbsp;</td></tr>,
                    ])}
                  </tbody>
                </table>
              </div>
              {processed.groups.length > 30 ? <p className="mt-2 text-xs text-ink-3">Showing first 30 groups. The downloaded workbook contains all {processed.groups.length} groups.</p> : null}
            </div>
          </section>
        ) : null}
      </main>
