      ...
      <div className="space-y-6">
        ...
        {tab === "engine" ? (
          <div className="ws-surface rounded-xl p-6">
            ...
            {queryRows.length ? <pre className="mt-4 max-h-72 overflow-auto rounded-xl bg-inverse p-4 text-xs text-on-inverse-2">{JSON.stringify(queryRows, null, 2)}</pre> : null}
          </div>
        ) : null}
        ...
        {tab === "spreadsheet" ? (
          <div className="ws-surface rounded-xl p-6">
            ...
            {spreadsheetResult ? <div className="mt-5 space-y-4"><div className="flex flex-wrap gap-2">{spreadsheetResult.sheetNames.map((name)=><span key={name} className="rounded-md bg-surface px-3 py-1 text-xs font-medium text-accent">{name}</span>)}</div><div><p className="text-xs font-medium uppercase tracking-wider text-on-inverse-3">Detected columns</p><div className="mt-2 flex flex-wrap gap-2">{spreadsheetResult.headers.map((header,index)=><span key={index} className="rounded-lg border bg-canvas px-2.5 py-1 text-xs text-on-inverse-2">{header || `Column ${index+1}`}</span>)}</div></div><pre className="max-h-64 overflow-auto rounded-xl bg-inverse p-4 text-xs text-on-inverse-2">{JSON.stringify(spreadsheetResult.preview, null, 2)}</pre></div> : null}
          </div>
        ) : null}
