        ...
        {processed && downloadInfo && validation?.ok ? (
          <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5">
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2">
              <button className="rounded-full bg-slate-950 px-6 py-3 text-sm font-semibold text-white transition hover:bg-teal-700 shadow-sm" onClick={download}>Download Sorted Workbook</button>
              <CopyDataButton
                payload={{
                  type: "records",
                  data: processed.details.map((d) => ({
                    Date: d.date || "",
                ...
                size="sm"
              />
              {processed.errors.length ? <button className="rounded-full border border-slate-300 px-5 py-3 text-sm font-semibold text-slate-700 transition hover:border-slate-950 shadow-sm" onClick={() => setShowErrors((s) => !s)}>{showErrors ? "Hide" : "View"} Error Report ({processed.errors.length})</button> : null}
            </div>
