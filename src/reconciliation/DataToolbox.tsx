    ...
    <div className="mt-4 overflow-hidden rounded-xl border border-slate-200 bg-white">
      <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50/90 px-3 py-2">
        <span className="text-xs font-semibold text-slate-600">
          Preview ({table.rows.length} rows &bull; {table.headers.length} columns)
        </span>
        <CopyDataButton
          payload={{ type: "dataTable", headers: table.headers, rows: table.rows }}
          showCsv={true}
          showTsv={true}
          showJson={true}
          buttonTheme="light"
...
function DownloadButtons({ table, name }: { table: DataTable; name: string }) {
  return (
    <div className="mt-4 flex flex-wrap items-center gap-2">
      <button className="rounded-full bg-teal-500 px-4 py-2 text-xs font-semibold text-slate-950 transition hover:bg-teal-300 shadow-sm" onClick={() => downloadTableExcel(table, `${name}.xlsx`, name)}>Download Excel</button>
      <button className="rounded-full border border-slate-300 bg-white px-4 py-2 text-xs font-semibold text-slate-700 transition hover:border-slate-950 shadow-sm" onClick={() => downloadTableCsv(table, `${name}.csv`)}>Download CSV</button>
      <CopyDataButton
        payload={{ type: "dataTable", headers: table.headers, rows: table.rows }}
        showCsv={true}
        showTsv={true}
