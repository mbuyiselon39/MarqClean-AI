      ...
      <main id="main-content" className="relative mx-auto max-w-6xl px-5 pb-20 pt-24 lg:px-8">
        <div className="ws-surface mb-6 flex flex-wrap items-start justify-between gap-4 rounded-2xl p-6">
          <div className="flex items-start gap-4">
            <span className="flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-[#5B4FFF]/20 to-[#8B5CF6]/20 text-2xl ring-1 ring-inset ring-white/10">📊</span>
            <div>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                ...
                <span className="ws-badge rounded-md px-2 py-0.5 text-[11px] font-semibold text-[#b9aeff]">Engine: MarqClean AI Structure Engine</span>
              </div>
            </div>
          </div>
        ...
        <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5">
          ...
          <div
            className="mt-3 rounded-xl border border-dashed border-slate-300 bg-slate-50 p-6 text-center transition hover:border-indigo-500"
          ...
          >
            ...
            <div className="mt-4 flex flex-wrap justify-center gap-2">
              <button className="rounded-full bg-slate-950 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-indigo-700" onClick={() => inputRef.current?.click()} disabled={busy}>
        ...
        {workbook && activeSheet ? (
          <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5">
            ...
            <button className="mt-5 rounded-full bg-slate-950 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-indigo-700" onClick={runPreview}>Preview Data</button>
                  ...
                  <table className="min-w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {preview.sampleRows.map((row, i) => (
                        <tr key={i}><td className="px-3 py-1.5 text-slate-700">{row.date || "-"}</td><td className="px-3 py-1.5 text-slate-700">{row.client || "-"}</td><td className="px-3 py-1.5 text-slate-700">{row.srn || "-"}</td><td className="px-3 py-1.5 text-slate-700">{row.amount}</td><td className="px-3 py-1.5 font-medium text-indigo-700">{row.key}</td></tr>
                      ))}
                    </tbody>
                  </table>
                </div>
        ...
        <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5">
          ...
          {testResult ? (
            <div className={`mt-3 rounded-xl p-3 text-sm ${testResult.validation.ok ? "bg-emerald-50 text-emerald-800" : "bg-red-50 text-red-700"}`}>
              ...
              {testResult.validation.ok ? <button className="mt-2 rounded-full bg-slate-950 px-4 py-1.5 text-xs font-semibold text-white transition hover:bg-indigo-700" onClick={() => downloadBytes(testResult.workbook.bytes, testResult.workbook.fileName)}>Download Test Workbook</button> : null}
            </div>
          ) : null}
        </section>
        ...
        {preview ? (
          <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5">
            ...
            <button className="mt-4 rounded-full bg-indigo-500 px-6 py-3 text-sm font-semibold text-slate-950 transition hover:bg-violet-300" onClick={execute} disabled={busy}>
        ...
        {validation && !validation.ok ? (
          <section className="mt-6 rounded-2xl border border-red-200 bg-red-50 p-5">
            <h2 className="text-lg font-semibold text-red-700">Workbook validation failed. The generated Excel file is invalid.</h2>
        ...
        {processed && downloadInfo && validation?.ok ? (
          <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <button className="rounded-full bg-slate-950 px-6 py-3 text-sm font-semibold text-white transition hover:bg-indigo-700" onClick={download}>Download Sorted Workbook</button>
