    ...
    <>
      ...
      <nav className="mx-auto flex max-w-[100rem] items-center justify-between gap-4 px-5 py-3.5 lg:px-8" aria-label="Primary navigation">
        ...
        <div className="hidden items-center gap-6 text-[13px] font-medium text-[#b0bacb] xl:flex">
          <a href={SIMPLE_NAV[0].path} className="transition hover:text-[#5B4FFF]" onClick={(e) => { e.preventDefault(); go(SIMPLE_NAV[0].path); }}>{SIMPLE_NAV[0].label}</a>
          <a href={SIMPLE_NAV[1].path} className="transition hover:text-[#5B4FFF]" onClick={(e) => { e.preventDefault(); go(SIMPLE_NAV[1].path); }}>{SIMPLE_NAV[1].label}</a>
          <a href={SIMPLE_NAV[2].path} className="transition hover:text-[#5B4FFF]" onClick={(e) => { e.preventDefault(); go(SIMPLE_NAV[2].path); }}>{SIMPLE_NAV[2].label}</a>
          ...
          <a href="/contact" className="transition hover:text-[#5B4FFF]" onClick={(e) => { e.preventDefault(); go("/contact"); }}>Contact</a>
        </div>
          ...
          <div className="flex flex-col gap-1 text-[15px] font-medium text-[#b0bacb]">
            {SIMPLE_NAV.map((item) => (
              <a
                key={item.label}
                href={item.path}
                className="rounded-lg px-3 py-2.5 transition hover:bg-white/5 hover:text-[#5B4FFF]"
...
const TONE_STYLES: Record<StatusTone, string> = {
  ...
  processing: "bg-[#5B4FFF]/10 text-[#b9aeff] border-[#5B4FFF]/40",
  success: "bg-[#F5B93D]/12 text-[#ffd781] border-[#F5B93D]/40",
...
export function StatusBadge({ tone, label }: { tone: StatusTone; label: string }) {
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-semibold ${TONE_STYLES[tone]}`}>
      <span className={`h-1.5 w-1.5 rounded-full ${tone === "processing" ? "animate-pulse bg-[#5B4FFF]" : tone === "success" ? "bg-[#F5B93D]" : tone === "warning" ? "bg-[#f59e0b]" : tone === "error" ? "bg-[#ef4444]" : "bg-[#b0bacb]"}`} />
      {label}
    </span>
  );
}
      ...
      <div className="relative mx-auto max-w-[110rem] px-5 pb-20 pt-24 lg:px-8">
        ...
        <div className="ws-surface flex flex-wrap items-start justify-between gap-4 rounded-2xl p-6">
          <div className="flex items-start gap-4">
            <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-[#5B4FFF]/20 to-[#8B5CF6]/20 text-2xl ring-1 ring-inset ring-white/10">{icon}</span>
            <div>
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2">
                ...
                {engine ? <span className="ws-badge rounded-md px-2 py-0.5 text-[11px] font-semibold text-[#b9aeff]">Engine: {engine}</span> : null}
              </div>
            </div>
          </div>
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      {tools.map((t) => (
        <button
          ...
          className={`group flex items-start gap-3 rounded-xl border p-4 text-left transition ${active === t.key ? "border-[#5B4FFF]/60 bg-[#5B4FFF]/10" : "border-white/8 bg-white/[0.03] hover:border-[#5B4FFF]/40 hover:bg-white/[0.06]"}`}
        >
