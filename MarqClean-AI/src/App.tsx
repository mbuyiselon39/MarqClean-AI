      ...
      <header className="mc-glass fixed left-0 right-0 top-0 z-30 border-b border-white/10 text-white">
        <nav className="mx-auto flex max-w-[100rem] items-center justify-between gap-4 px-5 py-3.5 lg:px-8" aria-label="Primary navigation">
          ...
          <div className="hidden items-center gap-6 text-[13px] font-medium text-white/65 xl:flex">
            <a href="/data-cleaner" className="transition hover:text-[#5B4FFF]" onClick={(e) => { e.preventDefault(); openWorkspaceTab("leads"); }}>Quick Data &amp; CSV Cleaner</a>
            <a href="/excel-automation" className="transition hover:text-[#5B4FFF]" onClick={(e) => { e.preventDefault(); navigateToPage("excel-automation"); }}>Excel Automation</a>
            <a href="/reconciliation-hub" className="transition hover:text-[#5B4FFF]" onClick={(e) => { e.preventDefault(); navigateToPage("reconciliation-hub"); }}>Reconciliation Hub</a>
            <a href="/data-toolbox" className="transition hover:text-[#5B4FFF]" onClick={(e) => { e.preventDefault(); navigateToPage("data-toolbox"); }}>Data Toolbox</a>
            <a href="/excel-academy" className="transition hover:text-[#5B4FFF]" onClick={(e) => { e.preventDefault(); navigateToPage("excel-academy"); }}>Free Excel Academy</a>
        ...
        {isMobileNavOpen ? (
          <div id="mobile-nav-panel" className="border-t border-white/10 bg-[#0a0b16]/98 px-5 py-4 xl:hidden">
            <div className="flex flex-col gap-1 text-[15px] font-medium text-white/80">
              <a className="rounded-lg px-3 py-2.5 transition hover:bg-white/5 hover:text-[#5B4FFF]" href="/data-cleaner" onClick={(e) => { e.preventDefault(); setIsMobileNavOpen(false); openWorkspaceTab("leads"); }}>Quick Data &amp; CSV Cleaner</a>
              <a className="rounded-lg px-3 py-2.5 transition hover:bg-white/5 hover:text-[#5B4FFF]" href="/excel-automation" onClick={(e) => { e.preventDefault(); setIsMobileNavOpen(false); navigateToPage("excel-automation"); }}>Excel Automation</a>
              <a className="rounded-lg px-3 py-2.5 transition hover:bg-white/5 hover:text-[#5B4FFF]" href="/reconciliation-hub" onClick={(e) => { e.preventDefault(); setIsMobileNavOpen(false); navigateToPage("reconciliation-hub"); }}>Reconciliation Hub</a>
              <a className="rounded-lg px-3 py-2.5 transition hover:bg-white/5 hover:text-[#5B4FFF]" href="/data-toolbox" onClick={(e) => { e.preventDefault(); setIsMobileNavOpen(false); navigateToPage("data-toolbox"); }}>Data Toolbox</a>
              <a className="rounded-lg px-3 py-2.5 transition hover:bg-white/5 hover:text-[#5B4FFF]" href="/excel-academy" onClick={(e) => { e.preventDefault(); setIsMobileNavOpen(false); navigateToPage("excel-academy"); }}>Free Excel Academy</a>
      ...
      <main id="main-content">
        ...
        <section id="free-tools" className="bg-[#0a0b16] py-20 text-white">
          <div className="mx-auto max-w-7xl px-5 lg:px-8">
            ...
            <div className="mt-12 grid gap-x-8 gap-y-5 text-sm sm:grid-cols-2 lg:grid-cols-3">
              {Object.values(SEO_TOOL_PAGES).map((page) => (
                <PageLink className="border-t border-white/10 pt-5 font-semibold text-white/85 transition hover:border-[#5B4FFF]/50 hover:text-[#B9AEFF]" key={page.slug} page={page.slug}>
                  <span className="mc-mono block text-[11px] uppercase tracking-[0.18em] text-[#5B4FFF]/70">{page.keyword}</span>
                  <span className="mt-2 block text-lg tracking-[-0.02em]">{page.h1}</span>
                </PageLink>
              ))}
            </div>
          </div>
