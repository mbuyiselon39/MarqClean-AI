  ...
  return (
    <div className="relative flex h-full w-full items-center justify-center">
      <div className="grid w-full max-w-md grid-cols-2 gap-3 font-mono text-[11px]">
        ...
        <div className="rounded-lg border border-[#5B4FFF]/30 bg-[#5B4FFF]/10 p-3">
          <div className="mb-2 text-[9px] uppercase tracking-widest text-[#b9aeff]">Cleaned</div>
          <div className="space-y-2 text-white/90">
            <div className="mc-anim-float truncate rounded bg-[#F5B93D]/15 px-1.5 py-0.5 ring-1 ring-[#F5B93D]/30">John Smith · Chicago</div>
            <div className="truncate rounded bg-[#F5B93D]/15 px-1.5 py-0.5 ring-1 ring-[#F5B93D]/30">Mary-Jane O'Brien · NYC</div>
            <div className="truncate rounded bg-[#F5B93D]/15 px-1.5 py-0.5 ring-1 ring-[#F5B93D]/30">bob@acme.com</div>
          </div>
        </div>
      </div>
      <div
        className="pointer-events-none absolute inset-y-6 left-1/2 w-px -translate-x-1/2 bg-gradient-to-b from-transparent via-[#5B4FFF] to-transparent"
    ...
    <div className="flex h-full w-full flex-col items-center justify-center gap-3">
      <div className="w-full max-w-sm overflow-hidden rounded-lg border border-white/10">
        <div className="grid grid-cols-[22px_repeat(4,1fr)] bg-white/[0.05] text-[9px] text-white/35">
          <div />
          {["A", "B", "C", "D"].map((c) => (
            <div key={c} className={`truncate border-l border-white/8 px-2 py-1 ${c === "C" ? "bg-[#5B4FFF]/20 font-semibold text-[#b9aeff]" : ""}`}>{c}</div>
          ))}
        </div>
        {rows.map((row, ri) => (
      ...
      <div className="mc-anim-float inline-flex items-center gap-1.5 rounded-md border border-[#5B4FFF]/30 bg-[#12142b] px-2.5 py-1.5 font-mono text-[10px] text-[#b9aeff] shadow-[0_8px_24px_rgba(91,79,255,0.3)]">
        <span className="h-1.5 w-1.5 rounded-full bg-[#F5B93D]" /> =SUBTOTAL(109,[Actual])
      </div>
    </div>
  );
}
    ...
    <div className="flex h-full w-full flex-col items-center justify-center gap-2.5">
      {rows.map((r) => (
        <div key={r.bank} className="grid w-full max-w-sm grid-cols-[1fr_auto_1fr] items-center gap-2 text-[10px]">
          <div className="truncate rounded-md border border-[#5B4FFF]/30 bg-[#5B4FFF]/10 px-2.5 py-1.5 text-white/90">{r.bank}</div>
          <div className="flex items-center justify-center">
            {r.ledger ? (
              <svg className="h-3.5 w-3.5 text-[#F5B93D]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
          ...
          <div className={`truncate rounded-md border px-2.5 py-1.5 ${r.ledger ? "border-[#F5B93D]/30 bg-[#F5B93D]/10 text-white/90" : "border-white/8 bg-white/[0.02] text-white/35"}`}>
            {r.ledger ?? "— no match —"}
          </div>
        </div>
      ))}
    ...
    <div className="flex h-full w-full items-center justify-center">
      <div className="w-full max-w-sm rounded-lg border border-white/10 bg-white/[0.02] p-3">
        ...
        <div className="mt-2 space-y-1">
          {rows.map((r) => (
            <div key={r.name} className={`flex items-center justify-between rounded-md px-2 py-1.5 ${r.active ? "bg-[#5B4FFF]/15" : ""}`}>
              <span className={`font-mono text-[11px] ${r.active ? "text-white" : "text-white/55"}`}>{r.name}</span>
              <span className={`text-[9px] uppercase tracking-widest ${r.active ? "text-[#b9aeff]" : "text-white/30"}`}>{r.tag}</span>
            </div>
          ))}
        </div>
      </div>
...
export default function HeroCarousel({ onCleanFile, onSampleLeads, onOpenReconciliation }: HeroActions) {
  ...
  const concepts: Concept[] = [
    {
      ...
      subhead: "Upload raw lead lists, CSVs, or Excel files. MarqClean AI formats names, corrects emails, and repairs phone numbers in seconds.",
      visual: <VisualDiff />,
    },
    {
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute -left-24 top-24 h-72 w-72 rounded-full bg-[#5B4FFF]/10 blur-3xl" aria-hidden="true" />
      <div className="pointer-events-none absolute -right-24 bottom-10 h-80 w-80 rounded-full bg-[#8B5CF6]/15 blur-3xl" aria-hidden="true" />
      ...
      <div className="relative mx-auto w-full max-w-[1400px] px-5 pb-16 pt-24 sm:pb-20 sm:pt-28 lg:px-8 lg:py-24">
        ...
        <div className="grid items-center gap-10 lg:grid-cols-12 lg:gap-8">
          ...
          <div key={`text-${current.key}`} className="mc-fade lg:col-span-5">
            <span className="mc-mono inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[11px] uppercase text-indigo-200">
              <span className="h-1.5 w-1.5 rounded-full bg-[#5B4FFF] shadow-[0_0_10px_#5B4FFF]" />
              {current.tag}
            </span>
            ...
            <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center">
              ...
              <button className="group inline-flex items-center gap-1.5 px-2 py-2.5 text-sm font-semibold text-indigo-200 transition hover:text-white" onClick={onOpenReconciliation}>
            ...
            <div className="mt-6 flex flex-wrap items-center gap-x-5 gap-y-2 text-xs text-white/45">
              <span className="inline-flex items-center gap-1.5">
                <span className="h-1.5 w-1.5 rounded-full bg-[#F5B93D]" /> No file ever leaves your browser
              </span>
              <span className="inline-flex items-center gap-1.5">
                <span className="h-1.5 w-1.5 rounded-full bg-[#F5B93D]" /> 40+ Excel functions built in
              </span>
            </div>

            <p className="mc-mono mt-6 text-[11px] uppercase text-white/35">A product of Vertex Stream Technologies, a division of Vertex Stream Group</p>
          </div>
          ...
          >
            <div className="absolute -inset-6 -z-10 rounded-[2.25rem] bg-gradient-to-br from-[#5B4FFF]/20 via-[#8B5CF6]/10 to-transparent blur-2xl" aria-hidden="true" />
